...
- `./tmp/workflow-artifacts/{mode}-intake.md` — issue goal, acceptance criteria, constraints, out-of-scope
- `./tmp/workflow-artifacts/{mode}-discovery.md` — findings relevant to this issue
- `./tmp/workflow-artifacts/{mode}-requirements.md` — functional + non-functional requirements (if produced)
- `./tmp/workflow-artifacts/{mode}-security-plan.md` — security constraints and controls the implementation must satisfy
- `./tmp/workflow-artifacts/{mode}-test-plan.md` — test stubs and coverage requirements the implementation must satisfy
- Any relevant ADRs in `docs/adr/`

## Required output: `./tmp/workflow-artifacts/{mode}-plan.md` (reset and scoped to this issue)
...
Ordered steps to write the tests defined in `./tmp/workflow-artifacts/{mode}-test-plan.md`.
...
Security controls from `./tmp/workflow-artifacts/{mode}-security-plan.md` must each map to at least one step here.
...
- [ ] All test stubs from `./tmp/workflow-artifacts/{mode}-test-plan.md` are covered in Test implementation steps
- [ ] All acceptance criteria from `./tmp/workflow-artifacts/{mode}-intake.md` map to an implementation step
- [ ] All security controls from `./tmp/workflow-artifacts/{mode}-security-plan.md` map to a step or test
- [ ] No files outside the touch list are modified
- [ ] Rollback path is confirmed viable
- [ ] Coverage verification command from `./tmp/workflow-artifacts/{mode}-test-plan.md` is included in Verification checklist

## After producing `./tmp/workflow-artifacts/{mode}-plan.md`
