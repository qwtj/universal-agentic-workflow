...
2. `uwf-project-intake` to perform Project Intake and produce `./tmp/workflow-artifacts/{mode}-intake.md` including goal, non-goals, constraints, success metrics, stakeholders, target environment, risk tolerance, and the intended work-breakdown strategy (milestones/epics, sprints, issues/user stories, tasks).
3. `uwf-core-discovery` to inspect the codebase and update the intake as needed, producing `./tmp/workflow-artifacts/{mode}-discovery.md`.
...
8. `uwf-project-timeline-planner` to produce the timeline roadmap (`./tmp/workflow-artifacts/{mode}-plan.md`) document in `docs`
9. `uwf-project-reviewer` to review the project plan, produce a fix list if needed, and recommend readiness for implementation when clean. Return to timeline planning on fixes.
...
- Never start implementation without a scoped `./tmp/workflow-artifacts/{mode}-intake.md` and `./tmp/workflow-artifacts/{mode}-plan.md` for the active issue.
